...
def mergeAndWrite(values, writer):
    ...
    for diplo in values["Diplo"]:
        ...
        for c1, c2 in zip(consequence_1s, consequence_2s):
            ...
            ##prob = "%.3f" %(diplo[4])
            prob = str(diplo[3])
            collatedrow = values["Variant"] + [diplo[0], allele_1, allele_2, prob] + c1 + [c2[2]]
...
def parsevcf(vcfFiles, foundersFile, dbname, allowedTranscriptsFile, limit):
    ...
    if flag:
        ...
        for vcfFile in vcfFiles:
            ...
            writer.write("\t".join(["variant_id", "pos", "strain", "allele_1", "allele_2", "gene_name", "transcript_name", "prob"]))
            for line in vcfFile:
...
if DEBUG:
    engine = "myisam"
    parsevcf("../../nd2016/data/vcf/rel1410/", "../../nd2016/output/tmp/afounderfile1a1d2f4c7b5e", "../output/parsedValues.txt", None,None,100000)
    ipdb.set_trace()
    x=1/0
