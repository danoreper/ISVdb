...
filterVCF$filterVcfsInDir = function(vcfDirIn, vcfDirOut,  tmpdir="./tmp", vcfExtensionIn, founders=NULL, bedfile=NULL, keepcsq=T, mc.cores=1)
{
    ...
    func = function(i)
    {
        vcf_file = vcfFiles[i]
        vcfout   = vcfOuts[i]
        filterVCF$filterVcf(vcf_file = vcf_file, vcfout = vcfout, tmpdir = tmpdir, founders = founders, bed = bedfile)
    }
